...
Medit_Western_FunCatch <- Medits_total |> 
  ...
  dplyr::select(c(3:4, 9:10, 12, 17, 22, 24:29, 32:33, 37:41, 47:50, 52:72)) |> 
...
# There was significant numbers of errors due to missing observed temperature -> Rm cell x year
Medit_Western_FunCatch_without_NA = Medit_Western_FunCatch |> 
  anti_join(Medit_Western_FunCatch |> group_by(HEX_ID, YEAR) |> 
              ...
              filter(NaN_count > 0) |> select(HEX_ID, YEAR), by = c("HEX_ID", "YEAR")) # Remove 59K obs (~20%)
