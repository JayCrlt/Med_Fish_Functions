#### Setting up          ----
library(dplyr) ; library(sf) ; library(lubridate) ; library(FNN) 
...
hex_lookup       <- data.frame(HEX_ID = Hexagonal_grid$grid.id, X = hex_coords[,1], Y = hex_coords[,2])

# Apply to dataset
MEDIT_TEMP$TEMP_imputed <- mapply(impute_temp, MEDIT_TEMP$HEX_ID, MEDIT_TEMP$Date, MEDIT_TEMP$Month)
